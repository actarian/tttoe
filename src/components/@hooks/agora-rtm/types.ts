...
declare class EventEmitter<TEventRecord = object> {
  ...
  on<P extends keyof TEventRecord, T>(this: T, event: P, listener: (...args: ListenerType<TEventRecord[P]>) => void): this;
  once<P extends keyof TEventRecord, T>(this: T, event: P, listener: (...args: ListenerType<TEventRecord[P]>) => void): this;
  off<P extends keyof TEventRecord, T>(this: T, event: P, listener: (...args: any[]) => any): this;
...
declare class LocalInvitation extends EventEmitter<RtmEvents.LocalInvitationEvents> {
  ...
  send(): void;
  cancel(): void;
  on<EventName extends keyof RtmEvents.LocalInvitationEvents>(eventName: EventName, listener: (...args: ListenerType<RtmEvents.LocalInvitationEvents[EventName]>) => any): this;
}
...
declare class RemoteInvitation extends EventEmitter<RtmEvents.RemoteInvitationEvents> {
  ...
  accept(): void;
  refuse(): void;
  on<EventName extends keyof RtmEvents.RemoteInvitationEvents>(eventName: EventName, listener: (...args: ListenerType<RtmEvents.RemoteInvitationEvents[EventName]>) => any): this;
}

export declare class RtmChannel extends EventEmitter<RtmEvents.RtmChannelEvents> {
  readonly channelId: string;
  sendMessage(message: RtmMessage, messageOptions?: SendMessageOptions): Promise<void>;
  join(): Promise<void>;
  leave(): Promise<void>;
  getMembers(): Promise<string[]>;
  on<EventName extends keyof RtmEvents.RtmChannelEvents>(eventName: EventName, listener: (...args: ListenerType<RtmEvents.RtmChannelEvents[EventName]>) => any): this;
}
...
export declare class RtmClient extends EventEmitter<RtmEvents.RtmClientEvents> {
  ...
  queryPeersBySubscriptionOption(option: keyof typeof RtmStatusCode.PeerSubscriptionOption): Promise<string[]>;
  createMediaMessageByUploading(payload: Blob, params?: { fileName?: string; description?: string; thumbnail?: Blob | undefined; messageType?: "FILE"; }, transHandler?: MediaTransferHandler): Promise<RtmFileMessage>;
  createMediaMessageByUploading(payload: Blob, params?: { width?: number; height?: number; fileName?: string; description?: string; thumbnail?: Blob | undefined; thumbnailWidth?: number | undefined; thumbnailHeight?: number | undefined; messageType?: "IMAGE"; }, transHandler?: MediaTransferHandler): Promise<RtmImageMessage>;
  downloadMedia(mediaId: string, transHandler?: MediaTransferHandler): Promise<Blob>;
  createMessage<T extends RtmMessage>(message: Partial<T>): T;
  on<EventName extends keyof RtmEvents.RtmClientEvents>(eventName: EventName, listener: (...args: ListenerType<RtmEvents.RtmClientEvents[EventName]>) => any): this;
}
